...
class Graph_encoder(AMDE):
    ...
    def __init__(self, node_features_1, edge_features_1, node_features_2, edge_features_2,
                 out_features, message_size=2, message_passes=25,
                 ...
                 out_depth=2, out_hidden_dim=100, out_layer_shrinkage=1.0, num_class=2, use_seq=True, use_graph=True
                 ):
        super(Graph_encoder, self).__init__(node_features_1, edge_features_1, node_features_2, edge_features_2,
                                            message_size, message_passes, out_features, num_class=num_class,
                                            use_seq=use_seq, use_graph=use_graph)
        ...
        for _ in range(edge_features_1):
            self.msg_nns_1.append(
                FeedForwardNetwork(node_features_1, [msg_hidden_dim] * msg_depth, message_size, dropout_p=self.droupout,
                                   bias=False))
            self.att_nns_1.append(
                FeedForwardNetwork(node_features_1, [att_hidden_dim] * att_depth, message_size, dropout_p=self.droupout,
                                   bias=False))
        for _ in range(edge_features_2):
            self.msg_nns_2.append(
                FeedForwardNetwork(node_features_2, [msg_hidden_dim] * msg_depth, message_size, dropout_p=self.droupout,
                                   bias=False))
            self.att_nns_2.append(
                FeedForwardNetwork(node_features_2, [att_hidden_dim] * att_depth, message_size, dropout_p=self.droupout,
                                   bias=False))
        ...
        out_layer_sizes = [round(out_hidden_dim * (out_layer_shrinkage ** (i / (out_depth - 1 + 1e-9)))) for i in
                           range(out_depth)]
