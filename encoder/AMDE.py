...
class AMDE(nn.Module):
    ...
    def __init__(self, node_fts_1, edge_fts_1, node_fts_2, edge_fts_2, message_size, message_passes, out_fts, num_class,
        ...
        if not self.use_graph and not self.use_seq:
            self.use_graph = True
            self.use_seq = True
        ...
        self.emb = Embeddings(self.input_dim_drug, self.emb_size, self.max_d, self.dropout_rate).to("cuda:0")
        self.d_encoder = Encoder_MultipleLayers(self.n_layer, self.hidden_size, self.intermediate_size,
                                                ...
                                                self.hidden_dropout_prob).to("cuda:0")
    ...
    def forward(self, adj_1, nd_1, ed_1, adj_2, nd_2, ed_2, de_1, de_2, mask_1, mask_2):
        ...
        if self.use_graph:
            ...
            node_batch_node_neighbours_1 = torch.zeros(node_batch_size_1, max_node_degree_1, self.node_fts_1).to(
                "cuda:0")
            node_batch_edges_1 = torch.zeros(node_batch_size_1, max_node_degree_1, self.edge_fts_1).to("cuda:0")
            ...
            node_batch_node_neighbour_mask_1 = torch.zeros(node_batch_size_1, max_node_degree_1).to("cuda:0")
            ...
            node_batch_node_neighbours_2 = torch.zeros(node_batch_size_2, max_node_degree_2, self.node_fts_2).to(
                "cuda:0")
            node_batch_edges_2 = torch.zeros(node_batch_size_2, max_node_degree_2, self.edge_fts_2).to("cuda:0")
            node_batch_neighbour_neighbour_indices_2 = torch.cat([torch.arange(i) for i in node_degrees_2]).to("cuda:0")
            edge_batch_node_batch_indices_2 = torch.cat(
                [i * torch.ones(degree) for i, degree in enumerate(node_degrees_2)]
            ).long().to("cuda:0")
            node_batch_node_neighbour_mask_2 = torch.zeros(node_batch_size_2, max_node_degree_2).to("cuda:0")

            node_batch_node_neighbour_mask_1[
                edge_batch_node_batch_indices_1, node_batch_neighbour_neighbour_indices_1] = 1
            ...
            node_batch_node_neighbour_mask_2[
                edge_batch_node_batch_indices_2, node_batch_neighbour_neighbour_indices_2] = 1
            ...
            for i in range(self.message_passes):
                ...
                hidden_nodes_2[node_batch_batch_indices_2, node_batch_node_indices_2, :] = self.update_2(
                    node_batch_nodes_2, messages_2)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_size, max_position_size, dropout_rate):
        ...
        self.word_embeddings = nn.Embedding(vocab_size, hidden_size).to("cuda:0")
        self.position_embeddings = nn.Embedding(max_position_size, hidden_size).to("cuda:0")

        self.LayerNorm = LayerNorm(hidden_size).to("cuda:0")
        self.dropout = nn.Dropout(dropout_rate).to("cuda:0")

    def forward(self, input_ids):
        b = torch.LongTensor(1, 2).to("cuda:0")
        ...
        words_embeddings = self.word_embeddings(input_ids).to("cuda:0")
        position_embeddings = self.position_embeddings(position_ids).to("cuda:0")
